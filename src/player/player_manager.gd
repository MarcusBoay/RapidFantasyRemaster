...
func connect_signals():
    PlayerSignals.player_hp_changed.connect(update_player_hp)
    PlayerSignals.player_mp_changed.connect(update_player_mp)
    PlayerSignals.player_level_changed.connect(update_player_level)
    PlayerSignals.player_exp_changed.connect(update_player_exp)
    PlayerSignals.player_limit_changed.connect(update_player_limit)
    PlayerSignals.player_strength_changed.connect(update_player_strength)
    PlayerSignals.player_wisdom_changed.connect(update_player_wisdom)
    PlayerSignals.player_defense_changed.connect(update_player_defense)
    PlayerSignals.player_gold_changed.connect(update_player_gold)
    PlayerSignals.player_equip_changed.connect(update_player_equip)
    PlayerSignals.player_magic_changed.connect(update_player_magic)
    PlayerSignals.player_limit_equip_changed.connect(update_player_limit_equip)

func update_player_hp(delta: int):
    stats.hp = min(stats.hp_max, stats.hp+delta)
    # TODO: probably should clamp max(0, ..)
    PlayerSignals.player_hp_new.emit(stats.hp)

func update_player_mp(delta: int):
    stats.mp = min(stats.mp_max, stats.mp+delta)
    # TODO: probably should clamp max(0, ..)
    PlayerSignals.player_mp_new.emit(stats.mp)

func update_player_hp_max(delta: int):
    stats.hp_max += delta
    # TODO: probably should clamp max(0, ..)
    PlayerSignals.player_hp_max_new.emit(stats.hp_max)

func update_player_mp_max(delta: int):
    stats.mp_max += delta
    # TODO: probably should clamp max(0, ..)
    PlayerSignals.player_mp_max_new.emit(stats.mp_max)

func update_player_level(delta: int):
    stats.level += delta
    # TODO: probably should clamp max(level, 5)
    PlayerSignals.player_level_new.emit(stats.level)

func update_player_exp(delta: int):
    stats.experience += delta
    # TODO: exp table
    PlayerSignals.player_exp_new.emit(stats.experience)

func update_player_limit(delta: int):
    limit += delta
    # TODO: should clamp min(limit, 100)
    PlayerSignals.player_limit_new.emit(limit)

func update_player_strength(delta: int):
    stats.strength += delta
    PlayerSignals.player_strength_new.emit(stats.strength)

func update_player_wisdom(delta: int):
    stats.wisdom += delta
    PlayerSignals.player_wisdom_new.emit(stats.wisdom)

func update_player_defense(delta: int):
    stats.defense += delta
    PlayerSignals.player_defense_new.emit(stats.defense)

func update_player_gold(delta: int):
    stats.gold += delta
    PlayerSignals.player_gold_new.emit(stats.gold)
...
func update_player_equip(new_equip: Item, equip_type: Globals.ItemType):
    ...
    PlayerSignals.player_equip_new.emit(null if is_unequip else new_equip, equip_type)

func update_player_magic(new_magic: PlayerAttack, idx: int):
    equips.magic[idx] = new_magic
    PlayerSignals.player_magic_new.emit(new_magic, idx)

func update_player_limit_equip(new_limit: PlayerAttack):
    equips.limit = new_limit
    PlayerSignals.player_limit_equip_new.emit(new_limit)
...
func use_item(item: Item):
    ...
    if inventory.items.size() == 0:
        PlayerSignals.player_items_empty.emit()
