...
func _connect_signals() -> void:
    ...
    PlayerSignals.player_items_empty.connect(_populate_items_container)
    ...
    PlayerSignals.player_limit_equip_new.connect(func(new_limit: PlayerAttack): attack_stat_container.populate_info(new_limit))
...
func _on_exit_button_pressed() -> void:
    SceneLoader.load_scene("res://assets/overworld/overworld.tscn")


### TEST
func _on_add_hp_pressed() -> void:
    PlayerSignals.player_hp_changed.emit(randi_range(-5, 5))
    PlayerSignals.player_mp_changed.emit(randi_range(-5, 5))
    PlayerSignals.player_level_changed.emit(randi_range(-5, 5))
    PlayerSignals.player_exp_changed.emit(randi_range(-5, 5))
    PlayerSignals.player_limit_changed.emit(randi_range(-5, 5))
    PlayerSignals.player_strength_changed.emit(randi_range(-5, 5))
    PlayerSignals.player_wisdom_changed.emit(randi_range(-5, 5))
    PlayerSignals.player_defense_changed.emit(randi_range(-5, 5))
    PlayerSignals.player_gold_changed.emit(randi_range(-5, 5))

func _on_minus_hp_pressed() -> void:
    PlayerSignals.player_hp_changed.emit(-3)
...
func _on_attacks_container_item_selected(index: int) -> void:
    ...
    if attack.type == Globals.PlayerAttackType.MAGIC:
        PlayerSignals.player_magic_changed.emit(attack, magic_equipped_container.currently_selected)
    elif attack.type == Globals.PlayerAttackType.LIMIT:
        PlayerSignals.player_limit_equip_changed.emit(attack)
    else:
